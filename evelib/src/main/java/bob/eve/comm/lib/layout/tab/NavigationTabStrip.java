/*
 * Create by BobEve on 17-12-18 下午7:09
 * Copyright (c) 2017. All rights reserved.
 *
 * Last modified 17-12-18 下午7:09
 */
...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
	...
	public NavigationTabStrip(final Context context, final AttributeSet attrs,
		...
		final TypedArray typedArray =
				context.obtainStyledAttributes(attrs, R.styleable.NavigationTabStrip);
		try {
			setStripColor(
			...
			setInactiveColor(typedArray.getColor(R.styleable.NavigationTabStrip_nts_inactive_color,
																					 DEFAULT_INACTIVE_COLOR));
			setActiveColor(typedArray.getColor(R.styleable.NavigationTabStrip_nts_active_color,
																				 DEFAULT_ACTIVE_COLOR));
			setAnimationDuration(
					typedArray.getInteger(R.styleable.NavigationTabStrip_nts_animation_duration,
			...
			String[] titles = null;
			try {
				final int titlesResId =
						typedArray.getResourceId(R.styleable.NavigationTabStrip_nts_titles, 0);
				titles = titlesResId == 0 ? null : typedArray.getResources()
																										 .getStringArray(titlesResId);
	...
	public void setmStripPadding(float mStripPadding) {
		this.mStripPadding = mStripPadding;
		requestLayout();
	}
	...
	private void updateIndicatorPosition(final float fraction) {
		...
		mStripLeft = mStartStripX + (mResizeInterpolator.getResizeInterpolation(fraction, mIsResizeIn) *
																 (mEndStripX - mStartStripX));
	...
	protected void onDraw(final Canvas canvas) {
		...
		mStripBounds.set(mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F),
										 mStripGravity == StripGravity.BOTTOM ? mBounds.height() - mStripWeight -
																														mStripPadding : mStripPadding,//0.0F,
										 mStripRight - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.0F),
										 mStripGravity == StripGravity.BOTTOM ? mBounds.height() - mStripPadding
																													: mStripWeight +
																														mStripPadding);//mStripWeight);
