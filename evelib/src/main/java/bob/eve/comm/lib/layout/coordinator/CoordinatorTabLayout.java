/*
 * Create by BobEve on 17-12-19 下午4:43
 * Copyright (c) 2017. All rights reserved.
 *
 * Last modified 17-12-18 下午6:58
 */

package bob.eve.comm.lib.layout.coordinator;
...
import bob.eve.comm.lib.layout.tab.OnEveCoordinatorTabLayoutLoadImageListener;
...
public class CoordinatorTabLayout extends CoordinatorLayout {
	...
	private OnEveCoordinatorTabLayoutLoadImageListener mLoadImageListener;
	...
	private void initView(Context context) {
		LayoutInflater.from(context)
									.inflate(R.layout.coordinatortablayout, this, true);
		initToolbar();
	...
	private void initWidget(Context context, AttributeSet attrs) {
		TypedArray typedArray =
				context.obtainStyledAttributes(attrs, R.styleable.CoordinatorTabLayout);

		TypedValue typedValue = new TypedValue();
		mContext.getTheme()
						.resolveAttribute(R.attr.colorPrimary, typedValue, true);
		int contentScrimColor =
				typedArray.getColor(R.styleable.CoordinatorTabLayout_contentScrim, typedValue.data);
		mCollapsingToolbarLayout.setContentScrimColor(contentScrimColor);

		int tabIndicatorColor =
				typedArray.getColor(R.styleable.CoordinatorTabLayout_tabIndicatorColor, Color.WHITE);
		mTabLayout.setSelectedTabIndicatorColor(tabIndicatorColor);

		int tabTextColor =
				typedArray.getColor(R.styleable.CoordinatorTabLayout_tabTextColor, Color.WHITE);
	 ...
	 * 		标题
	 *
	 * @return EveCoordinatorTabLayout
	 */
		...
		mTabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
			...
			public void onTabSelected(TabLayout.Tab tab) {
				mImageView.startAnimation(
						AnimationUtils.loadAnimation(mContext, R.anim.anim_alpha_dismiss));
				if (mLoadImageListener == null) {
					if (mImageArray != null) {
						mImageView.setImageResource(mImageArray[tab.getPosition()]);
					}
				} else {
					mLoadImageListener.loadHeaderImages(mImageView, tab);
				}
				if (mColorArray != null) {
					mCollapsingToolbarLayout.setContentScrimColor(
							ContextCompat.getColor(mContext, mColorArray[tab.getPosition()]));
				}
				mImageView.setAnimation(AnimationUtils.loadAnimation(mContext, R.anim.anim_alpha_show));
			}
	 ...
	 * 		设置OnEveCoordinatorTabLayoutLoadImageListener
	 *
	 * @return EveCoordinatorTabLayout
	 */
	public CoordinatorTabLayout setLoadImageListener(
			OnEveCoordinatorTabLayoutLoadImageListener loadImageListener) {
	...
	public CoordinatorTabLayout setTranslucentNavigationBar(@NonNull Activity activity) {
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
			...
			mToolbar.setPadding(0, DeviceUtils.getStatusBarHeight(activity) >> 1, 0, 0);
		}
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			activity.getWindow()
