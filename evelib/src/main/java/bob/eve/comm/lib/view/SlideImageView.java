/*
 * Create by BobEve on 17-12-18 下午2:49
 * Copyright (c) 2017. All rights reserved.
 *
 * Last modified 17-12-18 下午2:49
 */
...
public class SlideImageView extends View {
	...
	public SlideImageView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		final TypedArray typedArray =
				context.obtainStyledAttributes(attrs, R.styleable.SlidedImageView);
	...
	private void createSource() {
		...
		if (this.bitmap != null) {
			if (this.axis == Axis.HORIZONTAL) {
				this.bitmap = Bitmap.createScaledBitmap(this.bitmap, this.height * this.bitmap.getWidth() /
																														 this.bitmap.getHeight(), this.height,
																								false);
				this.slideSize = (this.bitmap.getWidth() - this.width) * (-1);
			} else {
				this.bitmap = Bitmap.createScaledBitmap(this.bitmap, this.width,
																								this.width * this.bitmap.getHeight() /
																								this.bitmap.getWidth(), false);
